...
def data_balanced(df,tokenizer):
    ...
    y_train = df['EmotionalPolarityClass']
...
for name in modelsname:
    if name == 'autodl-tmp/model/roberta-base':
        ...
        model = RobertaForSequenceClassification.from_pretrained(name, num_labels=len(empathy_groups))
    elif name == '/root/bert-base-uncased':
        tokenizer = BertTokenizer.from_pretrained(name)
        model = BertForSequenceClassification.from_pretrained(name, num_labels=len(empathy_groups))
    else:
        tokenizer = AutoTokenizer.from_pretrained(name)
        model = AutoModelForSequenceClassification.from_pretrained(name, num_labels=len(empathy_groups))
...
if  result > 0.73:
    ...
    # 保存为 TSV 文件
    logits_df_test.to_csv('boosting_bert_Empathy'+str(result)+'total_test_preds.csv', sep='\t', index=False)
    print("total_test_preds have been saved to Empathy"+str(result)+'total_preds.csv')
