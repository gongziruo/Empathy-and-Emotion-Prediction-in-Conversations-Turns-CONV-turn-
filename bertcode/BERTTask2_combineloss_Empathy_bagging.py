...
def train_model(train_loader):
    for epoch in range(epochs):
        ...
        # if (epoch+1) % 1 == 0:
        #     model.eval()
        #     _,result = ensemble_predict(model,NUM_MODELS)
        #     print("Dev eval result:", result)
    return model
...
def ensemble_predict(models):
    ...
    for batch in tqdm(dev_loader, desc="Evaluating"):
        ...
        vote_preds = torch.mode(batch_preds, dim=0).values
        total_preds = torch.cat((total_preds, vote_preds), dim=0)
    ...
    if  result > 0.63:
        # 如果 logits 在 GPU 上，确保转移到 CPU 并转换为 numpy 数组
        logits_np = total_preds.detach().cpu().numpy()
        # 创建 DataFrame
        logits_df = pd.DataFrame(logits_np)
        # 保存为 TSV 文件
        logits_df.to_csv('bagging_bert_Empathy'+str(result)+'total_preds.csv', sep='\t', index=False)
        print("total_preds have been saved to 'Empathy'"+str(result)+'total_preds.csv')

        torch.save(model, '/root/models/bagging_bert_Empathy' + str(result) + '.pth')
...
train_dataset = Task2Dataset(sampled_df, tokenizer)
# data_loader
...
# 对整个数据集进行最终预测
final_predictions,eval_res = ensemble_predict(models)
