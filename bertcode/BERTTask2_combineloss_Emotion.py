...
model = BertForSequenceClassification.from_pretrained('/root/bert-base-uncased', num_labels=len(emotion_groups))
...
# 定义模型和损失函数
num_classes = [9,5,11]
loss_func = CombinedLoss(num_classes=num_classes[0], alpha=0.8, beta=0.2,thegma =0.8)
def evaluate():
    model.eval()
    total_eval_accuracy = 0
    y_pred = torch.tensor([0, 0]).to(device)
    y_truth = torch.tensor([0, 0]).to(device)
    correct_count = torch.zeros(num_classes)
    total_count = torch.zeros(num_classes)
...
for epoch in range(epochs):
    ...
    if (epoch+1) % 1 == 0:
        max_correlation = 0.58
        eval_res = evaluate()

        print("Dev eval result:", eval_res)
        if eval_res[1]>max_correlation:
            torch.save(model, '/root/models/emotion' + str(eval_res) + "-" + str(epoch) + '.pth')
