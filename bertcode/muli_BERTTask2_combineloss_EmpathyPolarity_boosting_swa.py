...
#df = pd.read_csv("/root/mixed_train_EmotionalPolarity.csv",encoding='ISO-8859-1')
...
def train_model_with_swa(train_loader, model, optimizer, epochs, swa_start, swa_freq, lr_scheduler=None):
    # 创建 SWA 模型
        ...
        if epoch >= swa_start and (epoch - swa_start) % swa_freq == 0:
            ...
            with torch.no_grad():
                ...
                for batch in tqdm(dev_loader, desc="Evaluating"):
                    ...
                    outputs = swa_model(batch_inputs, attention_mask=batch_masks)
                    predicted = torch.argmax(outputs.logits, dim=-1)
                    total += labels.size(0)
                    correct += (predicted == labels).sum().item()
                    total_preds = torch.cat((total_preds, predicted), dim=0)
...
for name in modelsname:
    ...
    swa_model = train_model_with_swa(train_loader, model, optimizer, epochs, swa_start, swa_freq, lr_scheduler)
    print("SWA 训练完成")
    ###
