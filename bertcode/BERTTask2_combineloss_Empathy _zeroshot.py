...
# 定义模型和损失函数
num_classes = [9,5,11]
loss_func = CombinedLoss(num_classes=num_classes[2], alpha=0.8, beta=0.2,thegma =0.8)
def evaluate():
    model.eval()
...
for epoch in range(epochs):
    ...
    if (epoch+1) % 1 == 0:
        max_correlation = 0.63
        eval_res = evaluate()

        print("Dev eval result:", eval_res)
        if eval_res[1]>max_correlation:
            torch.save(model, '/root/models/Empathy' + str(eval_res) + "-" + str(epoch) + '.pth')
